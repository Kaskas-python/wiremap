...
2. **Module map is fixed.** `src/wiremap/{cli,discover,parse,frameworks,resolve,commands}.py`
   plus `queries/*.scm`. No new modules, packages or config files without a plan change.
...
6. **Test budget is one file.** `tests/test_wiremap.py`, the five tests in the plan, against
   the fixture repo in `tests/conftest.py`. Test lines never exceed production lines. Do
   not add tests; extend an existing one if a new assertion is truly needed.
